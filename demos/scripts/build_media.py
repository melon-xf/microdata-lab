...
def draw_tax_scene(data: SceneData, progress: float, size: tuple[int, int]) -> Image.Image:
    ...
    if callout_p > 0.15:
        ...
        c.text((910, 685), "The label changes. The deduction does not.", size=24, weight="bold")
        c.wrapped(
            (910, 724),
            (
                f"Counting the $9,325 average premium adds {adjusted - official:.1f} "
                "points—leaving the U.S. within one point of the Nordic average "
                f"({nordic_avg:.1f}%)."
            ),
            size=18,
            max_width=510,
...
def draw_power_scene(data: SceneData, progress: float, size: tuple[int, int]) -> Image.Image:
    ...
    draw_header(
        ...
        "Public power built access. It still charges less.",
...
def draw_energy_scene(data: SceneData, progress: float, size: tuple[int, int]) -> Image.Image:
    ...
    draw_header(
        ...
        "Oil jumped. Gas followed. Expectations didn’t.",
        (
    ...
    if phase(progress, 0.88, 0.97) > 0.15:
        ...
        c.text(
            (1000, 780),
            "Energy prices moved. Five-year inflation expectations did not follow.",
            size=20,
            weight="bold",
            anchor="mm",
        )
